...
function objective(company: Company): CompanyObjective {
  ...
  return {
    id: `${company.name}-objective`,
    title: company.objective,
    description: `Operate ${company.name} toward its active objective.`,
    priority: 10,
...
export default function CompanyRuntimeProvider({ company, children }: { company: Company; children: ReactNode }) {
  ...
  const counts = useMemo(() => ({
    completed: runtime.plan.tasks.filter(task => task.status === 'COMPLETED').length,
    executing: runtime.plan.tasks.filter(task => task.status === 'EXECUTING').length,
    blocked: runtime.plan.tasks.filter(task => task.status === 'BLOCKED').length,
  }), [runtime.plan.tasks])
...
export function useCompanyRuntime() {
  const value = useContext(RuntimeContext)
  if (!value) throw new Error('useCompanyRuntime must be used inside CompanyRuntimeProvider')
  return value
}
