  ...
  return (
    <main className="runtimeWorkspace">
      <aside className="runtimeSidebar">
        ...
        <div className="runtimeIdentity"><span>COMPANY</span><strong>{company.name}</strong><small>{company.type}</small><em><i /> Operational</em></div>
        ...
        <div className="runtimeSafety"><ShieldCheck size={15}/><span>Simulation mode<br/><b>External actions gated</b></span></div>
      </aside>
      <section className="runtimeMain">
        <header className="runtimeHeader"><div><span>COMPANY / {view.toUpperCase()}</span><h1>{view}</h1><p>Deterministic execution, measurable outcomes and controlled autonomy.</p></div><div className="runtimeHeaderStatus"><span><i /> CYCLE {runtime.plan.cycle}</span><button onClick={() => navigate('Settings')}><Settings size={15}/></button></div></header>
        ...
        {view === 'Security' && <Security runtime={runtime} />}
        ...
        {view === 'Settings' && <SimpleModule title="Settings" description="Runtime is simulation-first. External side effects remain explicitly gated." />}
        <footer>OBJECTIVE → MISSION → PLAN → AGENTS → EXECUTION → EVALUATION → LEARNING → REPLAN</footer>
      </section>
    </main>
  )
}
...
function CommandCenter({ company, runtime, running, setRunning, progress, counts, lastAction, advance, onView, assignedAgents }: any) {
  ...
  return <>
    ...
    <div className="runtimeGrid">
      ...
      <section className="runtimePanel runtimeWide"><PanelTitle title="Runtime Decision Log" action="Intelligence" onClick={() => onView('Intelligence')}/><div className="runtimeDecision"><CheckCircle2 size={15}/><div><strong>{lastAction}</strong><small>Execution state updated by the deterministic company runtime.</small></div></div><div className="runtimeDecision"><ShieldCheck size={15}/><div><strong>External actions remain gated</strong><small>Simulation mode prevents uncontrolled real-world side effects.</small></div></div></section>
    </div>
  </>
}
...
function Security({ runtime }: any) { return <Module title="Security & Control" description="Safety boundaries are independent from normal execution decisions."><div className="securityGrid"><div><ShieldCheck size={18}/><strong>External actions</strong><span>GATED</span></div><div><ShieldCheck size={18}/><strong>Risk approval</strong><span>{runtime.plan.tasks.some((t: any) => t.approvalRequired) ? 'REQUIRED' : 'CLEAR'}</span></div><div><ShieldCheck size={18}/><strong>Kill switch</strong><span>AVAILABLE</span></div></div></Module> }
