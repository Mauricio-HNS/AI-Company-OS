...
export default function CompanyMarketingPage({ params }: { params: { companyId: string } }) {
  ...
  return <main className="tenantMarketing">
    ...
    <section className="panel objectivePanel"><div className="title"><div><span>MARKETING MISSION</span><h2>O que você espera do Marketing?</h2></div><BrainCircuit size={18}/></div><p className="lead">Descreva o resultado que a empresa quer alcançar. O agente fará primeiro um diagnóstico de realidade antes de criar campanhas.</p><textarea value={objective} onChange={e => setObjective(e.target.value)} placeholder="Ex.: Quero aumentar minhas vendas em 200% nos próximos 30 dias, principalmente no período da tarde."/><div className="objectiveActions"><label>Horizonte <select value={horizonDays} onChange={e => setHorizonDays(Number(e.target.value))}><option value={7}>7 dias</option><option value={14}>14 dias</option><option value={30}>30 dias</option><option value={60}>60 dias</option><option value={90}>90 dias</option></select></label><button className="primaryAction" onClick={runRealityCheck}><Gauge size={15}/> Analisar viabilidade</button></div></section>
    ...
    <div className="grid"><section className="panel"><div className="title"><div><span>AUTONOMY POLICY</span><h2>Marketing capabilities</h2></div><ShieldCheck size={18}/></div><Policy label="Read marketing data" mode="AUTO"/><Policy label="Create campaigns" mode={createReady ? 'AUTO' : 'BLOCKED'}/><Policy label="Publish organic content" mode={organicReady ? 'OPTIONAL' : 'BLOCKED'}/><Policy label="Publish paid ads" mode="APPROVAL"/><Policy label="Change budget" mode="APPROVAL"/><div className="guard"><ShieldCheck size={14}/><span>Every company has its own provider accounts, policy and audit trail.</span></div></section><section className="panel"><div className="title"><div><span>EXECUTION SAFETY</span><h2>Budget & stop controls</h2></div><CircleDollarSign size={18}/></div><Metric label="Daily maximum" value="€20"/><Metric label="Monthly maximum" value="€300"/><Metric label="Current spend" value="€246"/><div className="meter"><i style={{ width: '82%' }} /></div><button className={`stop ${paused ? 'resume' : ''}`} onClick={toggleStop}>{paused ? <><Play size={14}/> Resume agents</> : <><Pause size={14}/> Emergency stop</>}</button></section></div>
