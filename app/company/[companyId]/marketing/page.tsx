...
import { ArrowLeft, Check, CircleDollarSign, Eye, Pause, Play, ShieldCheck, Target, X, PlugZap, History, BrainCircuit, Gauge } from 'lucide-react'
...
import { evaluateMarketingReality, type MarketingRealityCheck } from '../../../../lib/marketing-reality-check'
...
export default function CompanyMarketingPage({ params }: { params: { companyId: string } }) {
  ...
  const [reality, setReality] = useState<MarketingRealityCheck | null>(null)
  ...
  function runRealityCheck() {
    const percentMatch = objective.match(/([+]?\d+(?:[.,]\d+)?)\s*%/)
    const requestedGrowthPercent = percentMatch ? Number(percentMatch[1].replace(',', '.')) : 0
    const result = evaluateMarketingReality({
      requestedGrowthPercent,
      horizonDays,
      hasHistoricalCampaignData: state.campaigns.length > 0,
      historicalBestGrowthPercent: 18,
      storeReadiness: 55,
    ...
    setReality(result)
    setNotice(requestedGrowthPercent ? 'Reality Check concluído. O plano deve respeitar o diagnóstico antes da execução.' : 'Informe uma meta percentual no pedido para calcular a viabilidade.')
  }
  ...
  const levelLabel = reality?.level === 'VERY_LOW' ? 'MUITO BAIXA' : reality?.level === 'LOW' ? 'BAIXA' : reality?.level === 'MODERATE' ? 'MODERADA' : 'ALTA'
  const viabilityClass = reality?.level ? ` viability-${reality.level.toLowerCase()}` : ''

  return <main className="tenantMarketing">
    <header><Link href={`/company/${params.companyId}`}><ArrowLeft size={15} /> Company Command</Link><div><span>MARKETING / TENANT {params.companyId.toUpperCase()}</span><h1>Marketing Agent Workspace</h1><p>O Marketing pensa, questiona, planeja, executa sob políticas e aprende dentro do limite desta empresa.</p></div><div className="safe"><i /> ISOLATED</div></header>
    {notice && <div className="notice"><Check size={15} /> {notice}<button onClick={() => setNotice('')}><X size={13} /></button></div>}

    <section className="panel objectivePanel"><div className="title"><div><span>MARKETING MISSION</span><h2>O que você espera do Marketing?</h2></div><BrainCircuit size={18}/></div><p className="lead">Descreva o resultado que a empresa quer alcançar. O agente fará primeiro um diagnóstico de realidade antes de criar campanhas.</p><textarea value={objective} onChange={e => setObjective(e.target.value)} placeholder="Ex.: Quero aumentar minhas vendas em 200% nos próximos 30 dias, principalmente no período da tarde."/><div className="objectiveActions"><label>Horizonte <select value={horizonDays} onChange={e => setHorizonDays(Number(e.target.value))}><option value={7}>7 dias</option><option value={14}>14 dias</option><option value={30}>30 dias</option><option value={60}>60 dias</option><option value={90}>90 dias</option></select></label><button className={`primaryAction${viabilityClass}`} onClick={runRealityCheck}><Gauge size={15}/> {reality ? `Viabilidade: ${levelLabel}` : 'Analisar viabilidade'}</button></div></section>

    {reality && <section className={`panel reality ${reality.level.toLowerCase()}`}><div className="realityTop"><div><span>REALITY CHECK</span><h2>Viabilidade: {levelLabel}</h2></div><div className="confidence">Confiança da análise <b>{reality.confidence}</b></div></div><div className="realityGrid"><div><small>OBJETIVO SOLICITADO</small><strong>+{reality.requestedGrowthPercent}%</strong></div><div><small>META OPERACIONAL INICIAL</small><strong>+{reality.recommendedTargetPercent}%</strong></div><div><small>HORIZONTE</small><strong>{horizonDays} dias</strong></div><div><small>STATUS</small><strong>{reality.aspirational ? 'ASPIRACIONAL' : 'OPERACIONAL'}</strong></div></div><p className="rationale">{reality.rationale}</p><div className="realityColumns"><div><h3>Bloqueadores identificados</h3>{reality.blockers.length ? reality.blockers.map(item => <p key={item}>• {item}</p>) : <p>• Nenhum bloqueador crítico identificado pelos dados disponíveis.</p>}</div><div><h3>Próximas ações</h3>{reality.actions.map(item => <p key={item}>• {item}</p>)}</div></div><div className="evidence"><b>Evidências utilizadas</b>{reality.evidence.map(item => <span key={item}>{item}</span>)}</div></section>}
    ...
    <div className="pipeline"><Step n="01" title="Reality Check" state={reality ? levelLabel : 'WAITING'}/><Step n="02" title="Analyze" state={reality ? 'READY' : 'WAITING'}/><Step n="03" title="Strategy" state={reality ? 'READY' : 'WAITING'}/><Step n="04" title="Create" state={createReady ? 'READY' : 'WAITING CONNECTION'}/><Step n="05" title="Review" state="HUMAN GATE"/><Step n="06" title="Publish" state={organicReady ? 'POLICY CONTROLLED' : 'WAITING CONNECTION'}/><Step n="07" title="Measure" state="READY"/><Step n="08" title="Learn" state="READY"/></div>

    <div className="grid"><section className="panel"><div className="title"><div><span>AUTONOMY POLICY</span><h2>Marketing capabilities</h2></div><ShieldCheck size={18}/></div><Policy label="Read marketing data" mode="AUTO"/><Policy label="Create campaigns" mode={createReady ? 'AUTO' : 'BLOCKED'}/><Policy label="Publish organic content" mode={organicReady ? 'OPTIONAL' : 'BLOCKED'}/><Policy label="Publish paid ads" mode={paidNeedsApproval ? 'APPROVAL' : 'AUTO'}/><Policy label="Change budget" mode="APPROVAL"/><div className="guard"><ShieldCheck size={14}/><span>Every company has its own provider accounts, policy and audit trail.</span></div></section><section className="panel"><div className="title"><div><span>EXECUTION SAFETY</span><h2>Budget & stop controls</h2></div><CircleDollarSign size={18}/></div><Metric label="Daily maximum" value="€20"/><Metric label="Monthly maximum" value="€300"/><Metric label="Current spend" value="€246"/><div className="meter"><i style={{ width: '82%' }} /></div><button className={`stop ${paused ? 'resume' : ''}`} onClick={toggleStop}>{paused ? <><Play size={14}/> Resume agents</> : <><Pause size={14}/> Emergency stop</>}</button></section></div>
