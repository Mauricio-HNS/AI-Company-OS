...
function FiscalSection({ confirmed, onConfirm }: { confirmed: boolean; onConfirm: () => void }) {
  return <><small>IVA / DETECCIÓN DE ERRORES</small><h2>Revisión inteligente antes de presentar</h2><p>El sistema recalcula importes desde las facturas y compara cada valor con el formulario. Incluso una diferencia de 0,01 € queda marcada.</p>{checks.map(x => <div className={styles.row} key={x.label}><div>{x.ok ? <CheckCircle2 size={16}/> : <AlertTriangle size={16}/>}</div><span><b>{x.label}</b><small>{x.detail}</small></span><strong className={x.ok ? styles.good : ''}>{x.ok ? 'OK' : 'CORREGIR'}</strong></div>)}<div className={styles.callout}><AlertTriangle size={16}/><span>El período no debe presentarse mientras exista una inconsistencia.</span></div><button className={styles.primary} disabled={confirmed} onClick={onConfirm}>{confirmed ? 'Revisión confirmada' : 'Confirmar revisión'}</button></>
}
