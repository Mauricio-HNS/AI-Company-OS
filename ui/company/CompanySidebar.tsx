...
export default function CompanySidebar({ company, view, collapsed, onViewChange, onToggle }: { company: CompanySummary; view: CompanyView; collapsed: boolean; onViewChange: (view: CompanyView) => void; onToggle: () => void }) {
  return (
    <aside className={`${styles.sidebar} ${collapsed ? styles.collapsed : ''}`}>
      ...
      <div className={styles.sidebarBottom}><div className={styles.system}><i/>Simulation runtime online</div><Link href="/dash-1"><Building2 size={15}/>DASH 1</Link><button type="button" onClick={onToggle}>{collapsed ? <PanelLeftOpen size={16}/> : <PanelLeftClose size={16}/>}<span>{collapsed ? 'Expand menu' : 'Collapse menu'}</span></button></div>
    </aside>
  )
}
